...
function  calcularMedia ( )  {
  ...
  if (media === 0 ){
    ...
    mensagem=`Você obteve média de ${media}! Voçê está na média com ${média}`
  } else if  (media >= 7.1 && media <= 9.9) {
    mensagem=`Você obteve média de ${media}! Notão! Sua média é ${média}`
  } else {
    mensagem=`Você obteve média de ${media}!" Hoje é seu aniversário? Pq você tá de parabéns!10 de média"` 
   }
   const divConteudo=document.querySelector("#contentCalcMedias")
